...
function GuestBookingModification() {
	const bookingContext = useContext(BookingContext);
	const navigate = useNavigate()

	let param = useParams();
	let id = param.bookingId;
		...
		<div>
			{reservationDetails &&
				<div className="reservation-modif-container">
					...
					<div className="guest-reserv-modif-btn-div">
						<button className="guest-reserv-update-btn" disabled={availableTables} onClick={() =>{
							modifyReservation(id, { ...reservationDetails, status: false, date: `${day}T${time}` })
								...
								.then(() => navigate(`/users/dashboard/${userId}`))}
						}>
							Update reservation
						</button>
						<button className="guest-reserv-delete-btn"onClick={() => navigate(`/users/${userId}/booking/delete/${id}/${restId}`)}>
							Delete reservations
						</button>
					</div>
					...
					<button className="guest-reserv-back-btn"onClick={() => navigate(`/users/dashboard/${userId}`)}>
						Back
					</button>
...
export default GuestBookingModification;
